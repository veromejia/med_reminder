...
for task in session.query(Task).filter(Task.status_cd == "NEW0", Task.active == 1).all():
        ...
        job = cron.new(command='python3 /home/vero/Documents/medreminder_app/testPython/job.py',comment="medr{}".format(task.id_task))
        job.minute.every(1)
        cron.write()
        task.status_cd = "CRTD"
        session.commit()
