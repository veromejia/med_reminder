...
class PrescriptionForm(FlaskForm):
    ...
    frequency = StringField('Frequency',validators=[ DataRequired(),Length(min=1,max=2  ) ])
    ...
    def getPatients(self):
        if not self.patients.choices:
            ...
            for patient in db.all_patients():
                options.append( (patient.id,'{} {}'.format(patient.name,patient.last_name) ) )
    ...
    def printObj(self):
        return 'patient_id:{}, medication:{}, frequency:{}, start_dt:{}, end_dt:{}, noti_type:{} '.format(self.patients.data, self.medication.data, self.frequency.data, self.start_dt, self.end_dt.data, self.noti_type.data )
